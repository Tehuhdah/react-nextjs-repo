...
function FilteredEventsPage(props) {
  ...
  // Check if there is an error in the props
  // If there is, render an error message
  if (props.hasError) {
    return (
      <Fragment>
  ...
  // Create a new Date object using the year and month from the props
  const date = new Date(props.date.year, props.date.month - 1);

  // Render the ResultsTitle and EventList components
  return (
    <Fragment>
...
export async function getServerSideProps(context) {
  ...
  // Return the filtered events and the year and month in the props
  return {
    props: {
